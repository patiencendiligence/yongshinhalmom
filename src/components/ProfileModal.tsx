...
interface ProfileModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSelect: (profile: UserProfile, viewMode: "today" | "full" | "qna") => void;
...
export default function ProfileModal({ isOpen, onClose, onSelect, profiles, onDelete, lang }: ProfileModalProps) {
  ...
  const getSavedTodayQnaForProfile = (profile: UserProfile) => {
    ...
    try {
      ...
      if (
        ...
        u.birthTime === profile.birthTime &&
        u.gender === profile.gender &&
        u.birthPlace === profile.birthPlace
      ) {
        return parsed;
      }
    } catch (e) {
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-cream/90 dark:bg-black/90 backdrop-blur-sm">
          ...
          >
            <div className="p-8 border-b border-ink-black/10 dark:border-white/10 flex justify-between items-start bg-ink-black/[0.01] dark:bg-white/[0.02] relative z-10">
              ...
              <button onClick={onClose} className="p-3 hover:bg-ink-black/10 dark:hover:bg-white/10 transition-all border border-ink-black/10 dark:border-white/10 rounded-none">
            ...
            <div className="max-h-[55vh] overflow-y-auto p-4 space-y-6 no-scrollbar relative z-10">
              {profiles.length === 0 ? (
                ...
                profiles.map((profile) => {
                  const savedQna = getSavedTodayQnaForProfile(profile);
                  return (
                    <div
                      key={profile.id}
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="text-3xl font-serif font-black italic text-ink-black dark:text-white tracking-wide flex items-center gap-2.5 flex-wrap">
                          <span className="text-sm font-sans font-normal text-ink-black/40 dark:text-white/40 not-italic">
                            <Calendar className="w-4 h-4 text-ink-black/40 dark:text-white/40 inline shrink-0 mr-1.5 align-middle" /> 
                      ...
                      <div className="flex items-start gap-4 w-full md:w-auto shrink-0">
                        {savedQna ? (
                          /* If custom Q&A exists, show single button instead of today's fortune & full report buttons */
